...
- **Tool routing**: design, planning, hook and subagent work goes to Claude Code (more hook events, and it auto-delegates subagents). Long plan execution, second-opinion review and research-heavy pulls go to Codex (far fewer output tokens for the same work). Route by task type rather than picking a winner. `AGENTS.md` defers to this file rather than restating it, so this line is the single source.
...
- Commits are authored solely by the repository owner, so omit Co-Authored-By lines. This preserves clean authorship in the public commit history.
...
- After ~2 failed correction attempts on the same problem, `/clear` or `/rewind` rather than pushing on. Continuing against polluted context compounds the error instead of fixing it. Before clearing, write what was tried and why each attempt failed to a scratch file, and load that file into the fresh session with an explicit ask for new strategies: a full context favours repeating its own history, and a clean one without the log repeats it from scratch.
...
- When authoring a new command, agent, skill or CLAUDE.md rule, write the finished state rather than the method: what correct output looks like, what is explicitly out of scope, which constraints are hard, and what the agent can run to check itself. Keep a prescribed step order only where the order is a requirement of the result (a ledger written before an index, a scrub before a leak scan). In a controlled comparison a prescribed process ranked below unprescribed runs on design and test quality while burning 2.96x to 8.50x the tokens, because the agent optimised for the ritual instead of the outcome. This binds new instruction text; do not rewrite the existing order-dependent pipelines to match it.
...
### Integrations (GitHub; Jira template)

See [references/integrations.md](references/integrations.md) for MCP tool names and usage examples. **Jira is NOT wired up in this repo** (its MCP grants were removed 2026-08-10); that half of the reference is kept deliberately as a template for installs that do use it.
